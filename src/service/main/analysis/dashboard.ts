...
export function getGoodsCategoryCount() {
  return anRequest.get<IDataType>({
    url: 'cms/' + ChartsUrl.GOODS_CATEGORY_COUNT
  })
}
export function getGoodsCategorySale() {
  return anRequest.get<IDataType>({
    url: 'cms/' + ChartsUrl.GOODS_CATEGORY_SALE
  })
}
export function getGoodsCategoryFavor() {
  return anRequest.get<IDataType>({
    url: 'cms/' + ChartsUrl.GOODS_CATEGORY_FAVOR
  })
}
export function getGoodsAddressSale() {
  return anRequest.get<IDataType>({
    url: 'cms/' + ChartsUrl.GOODS_ADDRESS_SALE
  })
}

export function getGoodsAmountList() {
  return anRequest.get<IDataType>({
    url: 'cms/' + ChartsUrl.GOODS_AMOUNT_LIST
  })
}

export function getGoodsSaleTop10() {
  return anRequest.get<IDataType>({
    url: 'cms/' + ChartsUrl.GOODS_SALE_TOP10
  })
}
